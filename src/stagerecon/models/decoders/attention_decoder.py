"""Attention U-Net decoder with AttentionGate on skip connections."""
...
class AttentionDecoder(BaseDecoder):
    """Upsample path that gates encoder skips with additive attention."""
    ...
    def decode(
        ...
        """Upsample with attention-gated skip connections.
        ...
        x = bottleneck
        for up, attn, conv, skip in zip(self.ups, self.attentions, self.convs, skips):
            x = up(x)
            if x.shape[2:] != skip.shape[2:]:
                x = F.interpolate(x, size=skip.shape[2:], mode="nearest")
            gated_skip = attn(gate=x, skip=skip)
